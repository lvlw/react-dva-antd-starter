...
class Chart extends React.Component {
  ...
  render() {
    ...
    const gameCityPeopleCountProps = {
      ...
      data: game.map((item) => {
        ...
        city.forEach((c) => {
          res[c.name] = users.filter(n => n.gamesRanking.find(k => k.name === item)
                            && n.city === c.id).length;
        });
        return res;
      }),
      lines: city.map((item, index) => {
        return {
          name: item.name,
          color: colors[index],
          dataKey: item.name,
    ...
    return (
      <PageContent title="Chart">
        ...
        <Row gutter={24} style={{ marginBottom: 16 }}>
          <Card title="不同城市每种游戏的人数变化曲线">
            <LineChart {...gameCityPeopleCountProps} />
          </Card>
        </Row>
        <Row gutter={24} style={{ marginBottom: 16 }}>
          <Card title="Echarts">
            <Echart config={echartsOption} />
          </Card>
        </Row>
      </PageContent>
    );
